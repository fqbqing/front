...
define(function (require) {
    ...
    Vue.component('staff-item', {
         ...
         template: '<li>' +
                        '<div :class="{folder: isFolder,active: state.selectedId==model.id}"  class="group-item" @click="getGroupStaff">' +
                            ...
                            '<a href="javascript:void(0)" @click="updateGroup" v-if="model.parent_id!=state.topGroup.id && isAllow(\'user-group.update\')">修改</a>' +
                            '<a href="javascript:void(0)" @click="deleteGroup" v-if="model.parent_id!=state.topGroup.id && isAllow(\'user-group.delete\')">删除</a>' +
                            '<a href="javascript:void(0)" @click="updateGroupRole" v-if="isAllow(\'user-group.update-roles\')">角色分配</a></div>'+
                        '</div>' +
                        '<ul v-show="open" v-if="isFolder">' +
                            '<staff-item v-for="model in model.children" :level="level+1" :model="model" :state="state"></staff-item>' +
                        '</ul>' +
                    '</li>',
         props: {
             model: Object,
    ...
    Vue.filter('keyToValue', function(key){
        var vue = this;
        var item = u.find(vue.allRating,function(item){
    ...
    Vue.filter('customer-rating', function (key) {
        var item = u.find(consts.CUSTOMER_RATING_LIST, function (item) {
           return item.value === key;
        });
        return item ? item.text : '未分级';
    });
    ...
    Vue.filter('tuan-order-status', function (key) {
        return utils.getTuanOrderStatus(key);
    });

    Vue.filter('exhibition-status', function (key) {
        return utils.getExhibitionStatus(key);
    });

    Vue.filter('payment-type', function (key) {
    ...
    Vue.filter('activity-type', function (type) {
        var types = {
            1: '优惠券活动',
            2: '团购'
        };
    ...
    Vue.filter('trimwhitespace', function (text) {
        return utils.trimwhitespace(text).split('\n');
    });
