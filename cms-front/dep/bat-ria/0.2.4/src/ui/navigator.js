...
define(function (require) {
    ...
    Navigator.prototype.handleRedirect = function (e) {
        ...
        var activeClassName = 'nav-sub-item-current';
        u.each(document.getElementsByClassName(activeClassName), function (el) {
            lib.removeClass(el, activeClassName);
        });
            ...
            if (isActive(url, item)) {
                ...
                if (tabs.length && (!item.auth || permission.isAllow(item.auth))) {
                    ...
                    u.some(tabs, function (subItem, subIndex) {
                        if (subItem.tabs) {
                            var navs = subNavs[index].navItems[subIndex].subNavs;
                            return u.some(subItem.tabs, function (item, i) {
                                var el = navs[i];
                                if (isActive(url, item)) {
                                    activateNavElement(navs, el, i, activeClassName);
                                    return true;
                                }
                            });
                        }
    ...
    function toggleSubNav(navItems, element, index) {
        ...
        if (!element && u.isNumber(navItems.activeIndex)) {
            ...
            lib.removeClass(document.body, siderClassName);
        }
        else if (u.isNumber(index)) {
            if (!u.isNumber(navItems.activeIndex)) {
                navItems.activeIndex = index;
            ...
            lib.addClass(element, className);
            lib.addClass(document.body, siderClassName);
        }
    }
    ...
    function createNavElements(config, navItems, nav, isSub) {
        ...
        u.each(config, function (item, index) {
            if (!item.auth || permission.isAllow(item.auth)) {
                ...
                if (item.tabs && isSub) {
                    if (!item.auth || level.isAllow(item.auth)) {
                        element.innerHTML = '<div class="nav-sub-group-header"><span>' + item.text + '</span><i></i></div>';
                    }
                    else {
                        element.innerHTML = '<div class="nav-sub-group-header"><span>' + item.text + '</span>' +
                            '<small class="not-support" title="暂未开通该功能，请联系车大卖客服4008-955-088开通。">(未开通)</small><i></i></div>';
                    }

                    var ul = document.createElement('ul');
                    ul.className = 'nav-third';
                    element.appendChild(ul);
                    element.className = 'nav-' + isSub + 'item nav-' + isSub + 'group';
                    element.subNavs = {};
                    createNavElements(item.tabs, element.subNavs, ul, 'third-');
                }
                else {
