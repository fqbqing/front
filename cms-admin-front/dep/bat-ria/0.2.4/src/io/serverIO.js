...
define(function (require) {
     ...
     */
    io.hooks = {
        // 根据redirect的type字段生成与业务相关的跳转地址
        buildRedirectURL: function (redirect) {
            return redirect;
        }
    };
    ...
    var NET_CONNECTION_REFUSED = getGlobalError('网络请求被拒绝，请检查网络连接');
     ...
     */
    function requestSuccessHandler(options, rawData) {
        ...
        if ((data.success + '') !== 'true') {
            ...
            if (typeof message === 'string') {
            ...
            else if (typeof message.global !== 'undefined') {
                title = '系统提示';
                ...
                else if (typeof message.redirect === 'object') {
                    title = '系统提示';
                    content = message.redirect.info;
                    onok = function() {
                        var url = io.hooks.buildRedirectURL(message.redirect);
                        if (url) {
                            loc.assign(url);
                        }
                    };
            ...
            var result = data.page || data.result || data.data;
     ...
     * @return {meta.Promise} 处理后的Promise
     */
    function requestFailureHandler(options, fakeXHR) {
        var status = fakeXHR.status;

        var error;
        if (status === 0) {
            error = NET_CONNECTION_REFUSED;
        }
        ...
        return requestSuccessHandler(options, error);
    }
    ...
    io.request = function(url, data, options) {
        ...
        return ajax.request(options)
            .then(
                u.partial(requestSuccessHandler, options),
                u.partial(requestFailureHandler, options)
            );
    };
